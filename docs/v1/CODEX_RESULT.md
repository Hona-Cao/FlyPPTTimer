...
程序版本仍为 `1.13.0`；本轮包标识为 `v1.13.0-UX06-SettingsRules-20260909`。交付同名目录中的便携 `FlyPPTTimer.exe`，不要求解压或安装；包内使用默认配置，不携带临时调试配置、用户配置或日志。这是 review 手测版，不是正式 Release/Tag。
