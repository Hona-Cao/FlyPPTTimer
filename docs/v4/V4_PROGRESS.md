...
| 1 WPF 外壳/计时/多屏/大屏 | 进行中 | WPF 正式入口 | — |
...
阶段 1 精确任务：建立 WPF AppHost 与命令/状态边界；切换正式启动入口；用 TimerEngine 驱动 WPF 普通计时窗；抽取显示拓扑/定位并交付多屏和大屏；补真实 UIA、双 EXE Artifact、哈希、报告和 CI。阶段 1 已在阶段 0 CI 成功后自动开始，当前正在审计 `FlyPPTTimerContext`、overlay 和 big-screen 的运行时所有权与线程边界。
