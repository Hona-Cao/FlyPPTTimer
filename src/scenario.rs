use crate::config::{AppConfig, SCENARIO_BADGE_COLORS, ScenarioPreset, ScenarioSnapshot};
...
pub fn add_from_current(config: &mut AppConfig, name: String) -> Result<String, String> {
    if config.scenarios.len() >= 8 {
        return Err("最多只能保存 8 个情景模式。".into());
    }
...
pub fn save_current(config: &mut AppConfig, id: &str) -> Result<(), String> {
    let snapshot = ScenarioSnapshot::capture(config);
    let Some(item) = config.scenarios.iter_mut().find(|item| item.id == id) else {
        return Err("情景模式不存在。".into());
    };
...
pub fn apply(config: &mut AppConfig, id: &str) -> Result<(), String> {
    let Some(item) = config.scenarios.iter().find(|item| item.id == id).cloned() else {
        return Err("情景模式不存在。".into());
    };
    item.snapshot.apply_to(config);
    if config.timer.unlimited {
        config.timer.mode = crate::config::TimerMode::CountUp;
    }
    config.active_scenario_id = id.to_owned();
    Ok(())
}
...
pub fn remove(config: &mut AppConfig, id: &str) -> bool {
    ...
    if config.active_scenario_id == id {
        config.active_scenario_id.clear();
    }
...
mod tests {
    ...
    fn captures_applies_and_limits_presets() {
        ...
        c.timer.default_duration = "00:03:00".into();
        let id = add_from_current(&mut c, "竞赛模式".into()).unwrap();
        c.timer.default_duration = "00:20:00".into();
        apply(&mut c, &id).unwrap();
        assert_eq!(c.timer.default_duration, "00:03:00");
        for n in 1..8 {
            add_from_current(&mut c, format!("模式{n}")).unwrap();
        }
