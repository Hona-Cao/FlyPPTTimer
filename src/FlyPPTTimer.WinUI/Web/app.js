...
function renderPresentations(items){
  const host=$('presentationList'),signature=items.map(x=>[x.id,x.name,x.directory,x.isActive,x.isOpen,x.isSlideShow,x.duration,x.exists].join(':')).join('|');
  if(host.dataset.signature!==signature){
    ...
    items.forEach(item=>{
      const row=document.createElement('div');row.className='presentation-item';row.dataset.active=String(!!item.isActive);row.dataset.open=String(!!item.isOpen);row.dataset.exists=String(item.exists!==false);
      ...
      const status=item.isSlideShow?'正在放映':item.isActive?'当前活动':item.isOpen?'已打开':item.exists===false?'文件不存在':'待打开';
      meta.textContent=[item.directory,status,item.duration].filter(Boolean).join(' · ');meta.title=item.directory||'';
      text.append(title,meta);
      const actions=document.createElement('div');actions.className='presentation-actions';
      const open=document.createElement('button');open.dataset.presentationButton='true';open.dataset.active=String(!!item.isActive);open.textContent=item.isActive?'当前':item.isOpen?'切换':'打开';open.addEventListener('click',()=>command('ppt.openPresentation',{presentationId:item.id}));
      const beginning=document.createElement('button');beginning.dataset.presentationButton='true';beginning.textContent='从头';beginning.addEventListener('click',()=>command('ppt.openAndStartBeginning',{presentationId:item.id}));
      const current=document.createElement('button');current.dataset.presentationButton='true';current.textContent='当前页';current.addEventListener('click',()=>command('ppt.openAndStartCurrent',{presentationId:item.id}));
      if(item.exists===false){open.disabled=true;beginning.disabled=true;current.disabled=true;}
      actions.append(open,beginning,current);row.append(text,actions);host.append(row);
    });
  }
...
function refreshPresentationButtons(){
  document.querySelectorAll('[data-presentation-button]').forEach(button=>{
    const row=button.closest('.presentation-item'),missing=row?.dataset.exists==='false',active=button.dataset.active==='true';button.disabled=busy||!connected||missing||active;
  });
}
