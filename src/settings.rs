...
fn localize(lang: Language, value: &str) -> &str {
    ...
    match value {
        ...
        "名称" => "Name",
        "切换快捷键" => "Switch hotkey",
        "图标角标颜色" => "Icon badge color",
        "保存当前配置" => "Save current configuration",
        "更新此模式" => "Update this scenario",
        "切换到此模式" => "Switch to this scenario",
        "切换" => "Switch",
        "删除情景模式" => "Delete scenario",
        "基础计时" => "Basic Timer",
...
impl Row {
    ...
    fn duration_with_unlimited(value: impl Into<String>, unlimited: bool) -> Self {
        Self {
            value: value.into(),
            checked: unlimited,
            button: "无限制".into(),
            ..Self::new("timer.duration_pair", "默认时长 HH:mm:ss", 9)
        }
    }
...
fn other_rows(c: &AppConfig, lang: Language) -> Vec<Row> {
    vec![
        ...
        Row::section("软件更新"),
        Row::check("update.start", "启动时检测新版本", c.update.check_on_startup),
        Row::combo("update.source", "更新地址", vec!["Gitee", "GitHub"], c.update.source as i32),
        Row::info("中国大陆首次使用默认 Gitee，其他地区默认 GitHub", ""),
...
fn update_field(c: &mut AppConfig, key: &str, value: &str, checked: bool, selected: i32) {
    ...
    match key {
        ...
        "placement.bigmeta" => c.placement.big_screen_show_metadata = checked,
...
fn validate(c: &AppConfig, lang: Language) -> Result<(), String> {
    ...
    if !c.timer.unlimited && !crate::config::is_valid_duration(&c.timer.default_duration) {
        return Err(t(
            lang,
            "默认时长必须是 HH:mm:ss 格式。",
