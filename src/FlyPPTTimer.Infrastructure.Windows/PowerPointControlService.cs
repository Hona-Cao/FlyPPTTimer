...
public sealed class PowerPointControlService : IPowerPointControlService
{
    ...
    public PowerPointControlService(Func<AppConfig> getConfig, ILogService log)
    {
    ...
    public Task<PresentationCommandResult> ExecuteAsync(PresentationCommand command, CancellationToken cancellationToken = default) =>
        Task.Run(() => Execute(command), cancellationToken);

    public Task<string?> GenerateFirstSlideThumbnailAsync(string presentationId, string cacheDirectory, CancellationToken cancellationToken = default) =>
        Task.Run(() => Invoke(() => ExportFirstSlide(presentationId, cacheDirectory), TimeSpan.FromSeconds(20)), cancellationToken);
    ...
    private string ExecuteCore(PresentationCommand command)
    {
    ...
    private string? ExportFirstSlide(string presentationId, string cacheDirectory)
    {
        var path = GetAllowedPaths().FirstOrDefault(x => IdForPath(x) == presentationId);
        if (path is null || !File.Exists(path)) return null;
        Directory.CreateDirectory(cacheDirectory);
        var modified = File.GetLastWriteTimeUtc(path).Ticks;
        var output = Path.Combine(cacheDirectory, $"{IdForPath(path)}-{modified}.png");
        if (File.Exists(output)) return output;

        object? appObject = null, presentations = null, presentation = null, slides = null, slide = null;
        ...
        try
        {
            ...
            {
                presentation = ((dynamic)presentations).Open(path, true, false, false);
                openedForPreview = true;
            }
            slides = ((dynamic)presentation).Slides;
            if ((int)((dynamic)slides).Count < 1) return null;
            slide = ((dynamic)slides)[1];
            ((dynamic)slide).Export(output, "PNG", 640, 360);
            return File.Exists(output) ? output : null;
        }
        catch (Exception ex)
        {
            _log.Warn($"首张幻灯片缩略图生成失败：{FriendlyError(ex)}");
            return null;
        }
        finally
        {
            if (openedForPreview && presentation is not null) try { ((dynamic)presentation).Close(); } catch { }
            Release(slide, slides, presentation, presentations, appObject);
        }
    }
    ...
    private void AddRuleOptions(PresentationState state, IReadOnlyCollection<string> openPaths)
    {
        ...
        var rules = _getConfig().Rules.Where(x => x.Enabled).OrderBy(x => x.Order).ToList();
        var paths = rules.Select(x => x.FilePath).Concat(openPaths.Where(path => rules.All(rule => !SamePath(rule.FilePath, path))));
        foreach (var path in paths)
        {
            string fullPath;
            try { fullPath = Path.GetFullPath(path); } catch { continue; }
            var rule = rules.FirstOrDefault(x => SamePath(x.FilePath, fullPath));
            state.Presentations.Add(new PresentationOption
            {
                Id = IdForPath(fullPath), Name = Path.GetFileName(fullPath), Directory = Path.GetDirectoryName(fullPath) ?? "",
                IsOpen = openPaths.Any(x => SamePath(x, fullPath)), IsActive = SamePath(fullPath, activePath),
                IsSlideShow = state.IsSlideShowRunning && SamePath(fullPath, activePath), Duration = rule?.Duration ?? _getConfig().Timer.DefaultDuration,
    ...
    private List<string> GetAllowedPaths()
    {
        ...
        foreach (var rule in _getConfig().Rules.Where(x => x.Enabled && !string.IsNullOrWhiteSpace(x.FilePath)).OrderBy(x => x.Order))
        {
            try { result.Add(Path.GetFullPath(rule.FilePath)); } catch { }
        }
    ...
    private static PresentationState CloneState(PresentationState state) => new()
    {
        ...
        Error = state.Error,
        Presentations = state.Presentations.Select(x => new PresentationOption { Id = x.Id, Name = x.Name, Directory = x.Directory, IsOpen = x.IsOpen, IsActive = x.IsActive, IsSlideShow = x.IsSlideShow, Duration = x.Duration, Exists = x.Exists }).ToList()
    };
