...
public sealed class V0193FeatureTests
{
    ...
    public async Task GiteeLatestReleaseAndAttachmentsProduceAnAvailableInstallerUpdate()
    {
        const string releaseJson = """
            {"id":123,"tag_name":"v4.1.0","name":"FlyPPTTimer v4.1.0","body":"更新说明","html_url":"https://gitee.com/hona-cao/fly-ppttimer/releases/tag/v4.1.0"}
            """;
        const string assetsJson = """
            [
              {"id":456,"name":"FlyPPTTimer-v4.1.0-setup-win-x64.exe","browser_download_url":"https://gitee.com/hona-cao/fly-ppttimer/attach_files/456/download/setup.exe"},
              {"id":457,"name":"FlyPPTTimer-v4.1.0-portable-win-x64.zip","browser_download_url":"https://gitee.com/hona-cao/fly-ppttimer/attach_files/457/download/portable.zip"}
            ]
            """;
        ...
        Assert.Equal("4.1.0", result.Release!.Version);
        Assert.Equal("FlyPPTTimer-v4.1.0-setup-win-x64.exe", GiteeUpdateService.FindInstaller(result.Release)!.Name);
    }
